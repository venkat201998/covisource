...
const SlotsHistory = () => {
    ...
    return (
            <div className="col-lg-8 col-10 offset-lg-2 p-md-4 p-3">
                { historySlots.length > 0 ? historySlots.map((d) => d && <UserSlotCard key={d.patient && d.patient._id} hospital={d.hospital} patient={d.patient}/>): <h3 className="text-center">No Slots History</h3> }
            </div>
    );
};
